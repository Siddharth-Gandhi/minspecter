...
class IterableDataSetMultiWorker(IterableDataset):
    ...
    def ai2_to_transformers(self, data_instance, tokenizer):
        ...
        source_title = tokenizer(
            " ".join([str(token) for token in source_tokens]),
            truncation=True,
            padding="max_length",
            return_tensors="pt",
            max_length=512,
        ...
        source_input = {
            "input_ids": source_title["input_ids"][0],
        ...
        pos_title = tokenizer(
            " ".join([str(token) for token in pos_tokens]),
            truncation=True,
            padding="max_length",
            return_tensors="pt",
            max_length=512,
        ...
        pos_input = {
            "input_ids": pos_title["input_ids"][0],
        ...
        neg_title = tokenizer(
            " ".join([str(token) for token in neg_tokens]),
            truncation=True,
            padding="max_length",
            return_tensors="pt",
            max_length=512,
        ...
        neg_input = {
            "input_ids": neg_title["input_ids"][0],
    ...
    def forward(self, query, positive, negative):
        if self.distance == "l2-norm":
            ...
            distance_positive = torch.bmm(
                query.view(shapes[0], 1, shapes[1]), positive.view(shapes[0], shapes[1], 1)
            ).reshape(
                shapes[0],
            )
            distance_negative = torch.bmm(
                query.view(shapes[0], 1, shapes[1]), negative.view(shapes[0], shapes[1], 1)
            ).reshape(
                shapes[0],
            )
            losses = F.relu(-distance_positive + distance_negative + self.margin)
...
class Specter(pl.LightningModule):
    ...
    def test_epoch_end(self, outputs: list):
        ...
        embedding_output_list = [
            {"id": key, "embedding": value.detach().cpu().numpy().tolist()}
...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint_path", default=None, help="path to the model (if not setting checkpoint)"
    )
...
def main():
    ...
    if args.test_only:
        ...
        logger = TensorBoardLogger(save_dir=args.save_dir, version=0, name="pl-logs")

        # second part of the path shouldn't be f-string
        filepath = (
            f"{args.save_dir}/version_{logger.version}/checkpoints/"
        ...
        trainer = pl.Trainer(
            logger=logger, checkpoint_callback=checkpoint_callback, **extra_train_params
        )
